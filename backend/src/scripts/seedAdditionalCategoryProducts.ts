...
// Additional category products for Automotive, Books and Media, and Fashion
...
export const seedAdditionalCategoryProducts = async (): Promise<void> => {
  try {
    console.log('🚀 Starting additional category products seeding...');
    console.log('=====================================');
    ...
    const suppliers = {
      ...
      tech: await findOrCreateSupplier('TechHub Electronics', 'tech@techhub.com', 'Electronics and gaming equipment'),
      furniture: await findOrCreateSupplier('HomeStyle Furniture', 'info@homestyle.com', 'Home and office furniture'),
      health: await findOrCreateSupplier('HealthPlus Wellness', 'health@healthplus.com', 'Health and wellness products'),
      sports: await findOrCreateSupplier('SportMax Equipment', 'sports@sportmax.com', 'Sports and fitness equipment')
    };
    ...
    for (const productData of additionalCategoryProducts) {
      ...
      if (!existingProduct) {
        ...
        switch (productData.category) {
          ...
          case 'Fashion':
            ...
            break;
          case 'Electronics':
            supplierId = suppliers.tech._id;
            break;
          case 'Furniture':
            supplierId = suppliers.furniture._id;
            break;
          case 'Sports & Fitness':
            supplierId = suppliers.sports._id;
            break;
          default:
    ...
    console.log('\n🎉 Additional category products seeding completed!');
    console.log('📊 Summary:');
    console.log(`   - New products created: ${createdCount}`);
    console.log(`   - Existing products skipped: ${existingCount}`);
    console.log('   - Categories covered: Automotive (3), Books and Media (2), Fashion (3), Electronics (3), Furniture (1), Sports & Fitness (2)');
    console.log('   - Total products processed: 17');

  } catch (error) {
    console.error('❌ Error seeding additional category products:', error);
    throw error;
  }
};
...
if (require.main === module) {
  const runSeeding = async () => {
    try {
      ...
      await seedAdditionalCategoryProducts();
    } catch (error) {
      console.error('💥 Additional category products seeding failed:', error);
      process.exit(1);
    } finally {
